...
type HTTPClient interface {
    Do(config *HttpClientManager, body []byte, contentType types.ContentType) ([]byte, types.ContentType, error)
}
...
func (c *stdHTTPClient) Do(config *HttpClientManager, body []byte, contentType types.ContentType) ([]byte, types.ContentType, error) {
    ...
    return responseBody, GetContentTypeFromResponse(resp), nil
}
...
func (c *fastHTTPClient) Do(config *HttpClientManager, body []byte, contentType types.ContentType) ([]byte, types.ContentType, error) {
    ...
    responseContentType := parseContentTypeHeader(string(resp.Header.ContentType()))
    return resp.Body(), responseContentType, nil
}
...
func DoRequest[T any](payload any, config *HttpClientManager) result.Result[T] {
    ...
    // Execute request
    responseBody, responseContentType, err := client.Do(snap, bodyBytes, contentType)
    if err != nil {
    ...
    snap.Log.Debug(constant.TransactionSuccess,
        log.String("responseBody", string(responseBody)),
        log.String("responseContentType", responseContentType.String()),
    )
    ...
    // Decode response
    decodedResp, err := decodeResponse[T](responseBody, responseContentType, contentType)
    if err != nil {
...
// decodeResponse decodes HTTP response body using response Content-Type first,
// then the request Content-Type, then generic fallback decoders.
func decodeResponse[T any](body []byte, responseContentType, requestContentType types.ContentType) (T, error) {
    if responseContentType != "" {
        if decoded, err := decodeByContentType[T](responseContentType, bytes.NewReader(body)); err == nil {
            return decoded, nil
        }
    }

    if requestContentType != "" && requestContentType != responseContentType {
        if decoded, err := decodeByContentType[T](requestContentType, bytes.NewReader(body)); err == nil {
            return decoded, nil
        }
    }

    return fallbackDecode[T](bytes.NewReader(body))
}
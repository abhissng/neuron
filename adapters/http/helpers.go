...
// parseContentTypeHeader extracts the base media type from a Content-Type header value,
// stripping any charset or other parameter suffix after ';'.
func parseContentTypeHeader(contentType string) types.ContentType {
    contentType = strings.Split(contentType, ";")[0]
    return types.ContentType(strings.TrimSpace(contentType))
}
...
func GetContentTypeFromResponse(resp *http.Response) types.ContentType {
    return parseContentTypeHeader(resp.Header.Get("Content-Type"))
}
